...
export function UserForm({ user, isReadOnly = false }: UserFormProps) {
    ...
    const [formData, setFormData] = useState({
        ...
        voiceId: user?.voice?.voiceId || '',
        localModel: user?.llm?.localModel || 'gemma3:latest',
        cloudProvider: user?.llm?.cloudProvider || 'openai',
        cloudModel: user?.llm?.cloudModel || 'gpt-4o',
    });
        ...
        try {
            ...
            const payload = {
                ...
                avatar: {
                    ...
                    imageUrl: selectedAvatar?.preview || user?.avatar?.imageUrl,
                },
                voice: {
        ...
        <div className="space-y-8">
            ...
            <div className="flex flex-col md:flex-row gap-6 items-center md:items-start p-6 glass rounded-2xl">
                <div className="w-32 h-32 rounded-2xl bg-primary/10 border-2 border-white/10 shadow-2xl flex items-center justify-center overflow-hidden flex-shrink-0">
                    {avatars.find(a => a.id === formData.avatarName)?.preview || user?.avatar?.imageUrl ? (
                        <img
                            src={avatars.find(a => a.id === formData.avatarName)?.preview || user?.avatar?.imageUrl}
                            alt="Profile"
                ...
                <div className="flex-1 text-center md:text-left pt-2">
                    ...
                    <p className="text-muted text-sm flex items-center justify-center md:justify-start gap-2">
                        ...
                        {user?.avatar.configured ? `Avatar: ${user.avatar.avatarName}` : 'No Avatar Configured'}
                    </p>
                </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
                <div className="grid md:grid-cols-2 gap-8">
                    ...
                    <div className="glass rounded-2xl p-6 space-y-4">
                        ...
                        {(formData.avatarName || user?.avatar?.imageUrl) && (
                    ...
                    {user && (
                        <div className="glass rounded-2xl p-6 col-span-full">
                            ...
                            <p className="text-sm text-muted mb-4 italic">
                                RAG documents are managed in the <Link href="/setup/rag" className="text-primary hover:underline">RAG Setup</Link> section.
                            </p>
                        </div>
                    )}
                </div>
