...
export default function AvatarSetup() {
    ...
    const avatarReady = activeCandidate?.avatar.configured || false;
    ...
    const createAvatar = async () => {
        if (!imageFile || !activeCandidate) return;
    ...
    const saveManualAvatarName = () => {
        if (!manualAvatarName.trim() || !activeCandidate) return;
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-2xl font-bold">Avatar Setup</h1>
                    <p className="text-muted">
                        {activeCandidate
                            ? `Configure avatar for ${activeCandidate.name}`
                ...
                {isLoaded && !activeCandidate && (
                    <section className="glass rounded-2xl p-8 mb-8 border-2 border-warning text-center">
                        ...
                        <p className="text-muted mb-4">
                            Please select or create a candidate profile to configure avatar.
                        </p>
                            ...
                            <section className="glass rounded-2xl p-8 mb-8">
                                <div className="flex flex-col md:flex-row gap-8 items-center">
                                    <div className="w-48 h-48 rounded-2xl overflow-hidden glass">
                                        {activeCandidate.avatar.imageUrl ? (
                                            <Image
                                                src={activeCandidate.avatar.imageUrl}
                                                alt="Avatar"
                                                width={192}
                                                height={192}
                                    ...
                                    <div className="flex-1 text-center md:text-left">
                                        ...
                                        <p className="text-muted mb-1">
                                            Currently using <span className="font-mono text-xs">{activeCandidate.avatar.avatarName}</span>
                                        </p>
                                        <p className="text-sm text-muted mb-6">
                                            This avatar will be used in all live interview sessions for {activeCandidate.name}.
                                        </p>
                                        <button
                                            onClick={resetAvatar}
