...
export default function PracticePage() {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    if (data.defaultModel) {
                        ...
                        if (!localStorage.getItem('twinterview.settings')) {
    ...
    useEffect(() => {
        const stored = localStorage.getItem('twinterview.settings');
        if (stored) {
            try {
    ...
    useEffect(() => {
        if (settings.model) {
            localStorage.setItem('twinterview.settings', JSON.stringify({ llm: settings }));
        }
    }, [settings]);
    ...
    const saveToContext = async () => {
        if (!response || !currentQuestion || !activeCandidateId) return;
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-sm text-muted">Mock interviews with common questions</p>
                </div>
