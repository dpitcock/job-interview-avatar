...
export default function CandidateInterviewPage() {
    ...
    useEffect(() => {
        ...
        async function fetchSessions() {
            try {
                const res = await fetch(`/api/interview/sessions?userId=${activeUserId}`);
                if (res.ok) {
    ...
    const handleSend = async (e?: React.FormEvent) => {
        e?.preventDefault();
        if (!input.trim() || isStreaming || !currentSession || !activeUser) return;
        ...
        await stream(newMessages, {
            ...
            candidateInfo: { name: activeUser.name, role: activeUser.avatar.avatarName || 'Senior Engineer' },
            useRag: true
        });
    };
    ...
    const promoteToContext = async () => {
        if (!currentSession || !activeUserId) return;
        ...
        try {
            const res = await fetch('/api/interview/promote', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ sessionId: currentSession.id, userId: activeUserId }),
            });
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-background">
                <div className="text-center p-8 glass rounded-2xl max-w-md">
                    <h2 className="text-2xl font-bold mb-4">No Profile Selected</h2>
                    <p className="text-muted mb-6">Please select a user profile to start an interview session.</p>
    ...
    return (
        <div className="flex h-screen bg-background overflow-hidden">
            ...
            <aside className="w-80 border-r border-card flex flex-col glass z-10">
                    ...
                    {isInitialLoading ? (
                        ...
                        sessions.map(s => (
                            <button
                                ...
                                className={`w-full text-left p-4 rounded-xl transition-all duration-200 ${currentSession?.id === s.id
            ...
            <main className="flex-1 flex flex-col relative">
                {currentSession ? (
                    <>
                        {/* Chat Header */}
                        <header className="p-6 border-b border-card glass flex items-center justify-between z-10">
                            <div>
                                <h2 className="font-bold flex items-center gap-2">
                                    Interviewing <span className="text-primary">{activeUser?.name}</span>
                                </h2>
                        ...
                        >
                            {messages.length === 0 && (
                                <div className="h-full flex flex-col items-center justify-center text-center p-8">
                                    ...
                                    <p className="text-muted max-w-sm">
                                        The AI is ready to role-play as {activeUser?.name}.
                            ...
                            {messages.map((msg, i) => (
                                ...
                                >
                                    <div className={`max-w-[80%] rounded-2xl p-4 ${msg.role === 'user'
                                        ? 'bg-primary text-white shadow-lg glow-primary/20 rounded-br-none'
                                        : 'bg-card border border-card shadow-sm rounded-bl-none'
                                        }`}>
                                        <div className="text-xs opacity-60 mb-1 font-bold uppercase tracking-wider">
                                            {msg.role === 'user' ? 'Interviewer (You)' : activeUser?.name}
                                        </div>
                            ...
                            {isStreaming && (
                                <div className="flex justify-start">
                                    <div className="max-w-[80%] bg-card border border-card shadow-sm rounded-2xl rounded-bl-none p-4">
                                        <div className="text-xs opacity-60 mb-1 font-bold uppercase tracking-wider">
                                            {activeUser?.name}
                                        </div>
                                        <div className="whitespace-pre-wrap leading-relaxed text-sm">
                            ...
                            {error && (
                                <div className="flex justify-center">
                                    <div className="bg-error/20 text-error text-xs px-4 py-2 rounded-full border border-error/20">
                                        {error}
                                    </div>
                                </div>
                            )}
                        ...
                        {/* Input Area */}
                        <div className="p-6 glass border-t border-card z-10">
                            <form
                                onSubmit={handleSend}
                            ...
                            >
                                <input
                                    ...
                                    placeholder={`Ask ${activeUser?.name} a question...`}
                                    className="flex-1 bg-card border border-card rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all text-sm"
                                    disabled={isStreaming}
                                />
                                <button
                                    type="submit"
                                    disabled={!input.trim() || isStreaming}
                                    className="bg-primary text-white w-12 h-12 rounded-xl flex items-center justify-center hover:scale-105 active:scale-95 transition-all disabled:opacity-50 disabled:hover:scale-100 glow-primary"
                                >
                                    <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center text-center p-8 bg-card/10">
                        <div className="w-16 h-16 bg-card rounded-2xl flex items-center justify-center mb-4 shadow-sm border border-card">
